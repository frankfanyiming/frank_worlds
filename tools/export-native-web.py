...
parser.add_argument('--project',type=Path,help='Explicit isolated derivative project; source project is the default')
parser.add_argument('--mobile',action='store_true',help='Package smaller mobile chunks and label the profile')
args=parser.parse_args()
root=Path(__file__).resolve().parents[1]
project=args.project.resolve() if args.project else root/'worlds'/args.world/('source' if args.world=='frog' else 'web-project')
...
chunk_bytes=(6 if args.mobile else 16)*1024*1024
...
(output/'world-pack.json').write_text(json.dumps(dict(totalBytes=len(raw),sha256=hashlib.sha256(raw).hexdigest(),profile='mobile-speed' if args.mobile else 'desktop-visual',chunks=chunks),indent=2)+'\n')
...
shell=shell.replace('</head>',f'<meta name="xlands-source-commit" content="{commit}"></head>',1)
...
for source_name,public_name in [('SOURCE.json','SOURCE.json'),('LICENSE','LICENSE.txt'),('NOTICE','NOTICE.txt')]:
 (output/public_name).write_bytes((root/source_name).read_bytes())
