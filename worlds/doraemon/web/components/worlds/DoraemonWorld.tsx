...
export default function Page({ sound = false, locale = 'zh-CN' }: { sound?: boolean; locale?: Locale }) {
  ...
  const [s, setS] = useState(initial),
    [panel, setPanel] = useState<'settings' | 'pocket' | null>(null),
      ...
      .then(({ TownEngine }) => {
        ...
        try {
          ...
          setS((v) => ({
            ...v,
            error:
              '无法启动 3D 画面。请使用支持 WebGL 2 的浏览器，并开启图形加速。',
          }));
        }
      })
      .catch((e) => {
    ...
    <main className={'town-app ' + (s.mode === 'first' ? 'first-person' : '')}>
      ...
      <header className="topbar">
        <div className="camera-switch">
          <Tabs
            value={s.mode}
            onValueChange={(v) =>
              engine.current?.setMode(v as 'orbit' | 'first')
            }
          >
            <TabsList aria-label="探索视角">
        ...
        >
          ...
          <div>
            <b>{s.weather?.time ?? '15:30'}</b>
            <span>
              {(SEASONS.find((v) => v.id === s.weather?.season)?.name ?? '夏') +
                '日 · ' +
                (s.weather?.night ? '夜色' : '晴空')}
            </span>
          </div>
          <i />
        </button>
      ...
      {s.ready && (
        <>
          <nav className="side-tools" aria-label="探索工具">
            ...
            <button
              className={s.cutaway ? 'active' : ''}
              onClick={() => engine.current?.toggleCutaway()}
              title="查看大雄家剖面"
              aria-pressed={s.cutaway}
            >
              <Layers />
              <span>剖面</span>
            </button>
            <button onClick={() => engine.current?.showTown()} title="鸟瞰全镇">
              <Maximize />
              <span>全镇</span>
            </button>
          ...
          {(s.inside || s.cutaway) && s.mode === 'orbit' && (
            <div className="floor-switch">
              <span>{s.house === 'shizuka' ? '静香家' : '野比家'}</span>
              <button
                className={s.floor === 0 ? 'chosen' : ''}
                onClick={() => engine.current?.setFloor(0)}
              >
                一楼
              </button>
              <button
                className={s.floor === 1 ? 'chosen' : ''}
                onClick={() => engine.current?.setFloor(1)}
              >
                二楼
              </button>
            </div>
      ...
      {s.ready && s.bedroom?.requested && (
        <div className="room-loading" role="status" aria-live="polite">
          ...
          <p>{s.bedroom.error ?? '稍等一下，窗帘、榻榻米和书桌正在就位。'}</p>
          {s.bedroom.status !== 'error' && (
            <>
          ...
          <div>
            {s.bedroom.status === 'error' && (
              <button
                ...
                onClick={() => engine.current?.retryBedroomEntry()}
              >
                重试
              </button>
            )}
            <button
              ...
              onClick={() => engine.current?.cancelBedroomEntry()}
            >
              先逛小镇
            </button>
          </div>
      ...
      {!s.ready && !s.error && (
        <div className="loading-screen">
          <div className="loading-orbit" aria-hidden="true">
            <img
              src={assetPath('/doraemon-loading.png')}
              alt=""
              width="80"
              height="80"
            />
          ...
          <h2>来一起共建有趣的小世界吧</h2>
          <p role="status">{s.loadingStage ?? '正在打开小世界'}</p>
          <Progress
            value={s.progress}
            className="load-progress"
            aria-label="小镇加载进度"
          />
          <span className="load-percent">{s.progress}%</span>
          <button
            className="text-action"
            onClick={() => {
      ...
      {s.error && (
        <div className="loading-screen">
          ...
          <p>
            如果在聊天应用内打不开，可复制网址到 Safari、Chrome 或
            Edge。若仍无响应，尝试切换 Wi-Fi / 移动网络。
          </p>
        </div>
      )}
      <Pocket
        ...
        <DialogContent className="dialogue-box">
          <DialogHeader>
            <div className="dialogue-name">
              ...
              <DialogTitle>{s.actor?.name}</DialogTitle>
            </div>
            <DialogDescription className="dialogue-line">
          ...
          <div className="dialogue-footer">
            ...
            <button
              className="primary-action"
              onClick={() => {
                if (s.actor && dialogLine < s.actor.lines.length - 1)
                  setDialogLine((v) => v + 1);
                else engine.current?.closeDialogue();
              }}
            >
              {s.actor && dialogLine < s.actor.lines.length - 1
                ? '再聊一句'
                : '继续散步'}
              <ArrowRight size={16} />
            </button>
      ...
      <Dialog open={help} onOpenChange={setHelp}>
        <DialogContent className="settings-dialog">
          ...
          <div className="help-lines">
            <p>
              <kbd>W A S D</kbd>或方向键行走，按住 Shift 跑步
            </p>
            <p>
              <kbd>鼠标拖动</kbd>转动视角，俯视时滚轮缩放
            </p>
            <p>
              <kbd>点击地面</kbd>向那个位置行走，遇到障碍停下
            </p>
            <p>
              <kbd>V</kbd>切换第一人称与 45° 视角
            </p>
            <p>
              <kbd>E</kbd>与身边的人聊天、进屋、上下楼
            </p>
            <p>
              <kbd>R</kbd>查看大雄家的屋内剖面
            </p>
            <p>
              <kbd>空格 / Ctrl</kbd>竹蜻蜓上升 / 下降，C 安全着陆
            </p>
            <p>
              <kbd>空格</kbd>棒球挑战里挥棒或投球
            </p>
            <p>靠近门口按 E 进出房屋；口袋里的任意门可以前往其他地点。</p>
          </div>
        </DialogContent>
      </Dialog>
    </main>
