...
for(const name of ['world','loading','traffic','model-manifest']){const input=await readFile(new URL('../lib/town/'+name+'.ts',import.meta.url),'utf8');await writeFile(new URL(name+'.js',dir),ts.transpileModule(input,{compilerOptions:{target:ts.ScriptTarget.ES2022,module:ts.ModuleKind.ESNext}}).outputText);}
...
const {WORLD_DATA_FILE}=await import(new URL('model-manifest.js',dir));
const data=JSON.parse(await readFile(new URL('../public/models/'+WORLD_DATA_FILE,import.meta.url)));
...
const routes=[['home living',[-11.5,1.8],[-14.0,2.1],.48],['home kitchen',[-11.5,1.8],[-12.0,6.5],.48],['home stairs',[-11.5,1.8],[HOUSE.stairX,HOUSE.stairY0-.05],.48],['restored bedroom doorway',[HOUSE.stairX,HOUSE.stairY1+.4],[-10.775,3.3],3.15],['shizuka living',[-10.78,-21],[-9.72,-20.2],.24],['shizuka kitchen',[-10.78,-21],[-12.2,-16.2],.24],['shizuka stairs',[-10.78,-21],[-12.13,-20.55],.24]];
...
console.log('PASS: progress bounds/monotonicity, concurrency, timeout, HTTP failure, two staircases, seven room routes including the original bedroom doorway, car pedestrian braking and collision.');
