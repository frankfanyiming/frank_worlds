...
wy(-13.02,.4,5.7,.48,gap=(1.4,2.65));wy(-10.82,.4,5.85,.48,gap=(2.55,3.65));wx(-17.05,-13.02,5.70,.48,gap=(-14.8,-13.5))
...
G='v11_home_stairs';stairs(-12.48,3.05,6.50,.48,3.15,.80)
...
G='v11_home_upper';floor(-17.15,-13.0,.35,6.85,3.15);floor(-12.0,-8.95,.35,6.85,3.15);floor(-13,-12,.35,3.05,3.15);floor(-13,-12,6.5,6.85,3.15)
# Nobita's east room preserves the desk and time-machine location.
tatami(-12.88,-9.08,.52,5.62,3.17);tatami(-16.95,-13.20,.52,5.62,3.17)
wy(-17.15,.35,6.85,3.15);wy(-8.95,.35,6.85,3.15);wx(-17.15,-8.95,6.85,3.15);wy(-13.1,.4,5.65,3.15)
wx(-17.05,-13.1,5.72,3.15,gap=(-14.7,-13.6));wx(-12.05,-9.02,5.72,3.15,gap=(-12.0,-10.9))
# keep stairwell open, protect upper floor edge.
for yy0,yy1 in [(3.05,5.55)]:rod('upper stair guard',(-13.02,yy0,4.05),(-13.02,yy1,4.05),.035,'wood')
shelf(-12.70,2.9,3.17,.85,1.7);box('low cabinet',-12.25,2.04,3.46,.85,.52,.57,'wood',.035,True)
# Desk with real shallow drawer clearance.
table(-10.75,1.12,3.17,1.22,.68,.68);chair(-10.75,2.06,3.17,'green');books(-10.83,1.2,3.90,6);box('open notebook',-11.03,.90,3.903,.30,.22,.022,'paper');rod('desk lamp',(-11.18,1.20,3.9),(-11.18,1.20,4.18),.025,'blue');ell('lamp shade',-11.15,1.12,4.18,.15,.09,.06,'blue')
# Closet: frame, solid shelf, separated folded bedding and pillow.
box('closet back',-9.66,5.52,4.30,1.33,.07,2.30,'wood')
for xx in [-10.35,-8.99]:box('closet side',xx,5.02,4.30,.075,1.08,2.30,'wood')
for zz in [3.20,4.28,5.42]:box('closet shelf',-9.67,5.02,zz,1.36,1.08,.085,'floor')
for i in range(4):
 zz=4.38+i*.115;box('folded futon layer',-9.68,5.02,zz,1.05,.80,.095,'linen' if i%2 else 'cream',.043)
 for sign in [-1,1]:rod('folded quilt piping',(-10.12,5.02+sign*.34,zz),(-9.24,5.02+sign*.34,zz),.008,'white')
box('blue dotted cover',-9.68,5.02,4.86,1.0,.76,.12,'blue',.05)
for i in range(4):
 for j in range(3):ell('cover dot',-10.01+i*.22,4.76+j*.22,4.923,.035,.035,.003,'white')
pillow(-9.68,5.03,5.05,.63,.38)
box('wardrobe',-12.42,1.02,4.12,.78,.61,1.9,'wood',.02,True)
bed(-15.7,2.0,3.17)
G='v11_home_upper_front';wx(-17.15,-8.95,.35,3.15);window(-10.75,.25,4.58,1.95,1.14,curtain=True);window(-15.2,.25,4.58,1.95,1.14,curtain=True);window(-8.86,2.65,4.58,1.35,side=True,curtain=True)
door('v11_nobita_slide',-11.44,5.73,3.17,1.04,2.0,'paper',True)
# Closet track behind the separate panel, offset depth prevents z-fighting.
G='v11_home_upper'
for zz in [3.22,5.40]:
 for yy in [4.43,4.52]:box('double sliding rail',-9.67,yy,zz,1.43,.035,.035,'trim',.002)
door('v11_closet_slide',-9.3,4.46,3.23,.62,2.1,'paper',True)
...
def export_groups(path):
 ...
 return exports
export_groups(OUT/'neighborhood-v11.glb')
(OUT/'neighborhood-v11.json').write_text(json.dumps({'colliders':colliders,'home':{'entry':[-11.5,.35],'stairs':[-12.48,3.05,6.50]},'shizuka':{'entry':[-10.775,-22],'stairs':[-12.13,-20.5,-17.3]}}))
# Native file contains editable original objects plus named runtime batches.
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'tools/neighborhood-v11.blend'))
