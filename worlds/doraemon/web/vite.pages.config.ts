...
export default defineConfig({
 ...
 plugins:[react(),{name:'ship-runtime-assets-only',closeBundle(){
  ...
  const startupNames=new Set([...STARTUP_PARTS.map(name=>name+'.glb'),WORLD_DATA_FILE]);
  ...
  const mobileStartupNames=new Set([...STARTUP_PARTS.map(name=>modelFile(name,true)+'.glb'),WORLD_DATA_FILE]);
