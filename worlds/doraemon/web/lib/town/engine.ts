...
export class TownEngine {
 ...
 groundSurfaces:GroundSurface[]=[]; assetRevision=0; colliders:Collider[]=[]; slidingDoors:{object:THREE.Object3D;collider:Collider;baseX:number;travel:number;target:number}[]=[]; keys=new Set<string>(); pointer={down:false,x:0,y:0,moved:0}; paused=false; touch={x:0,y:0};
 ...
 async load(){try{
  ...
  const dataTask=fetchBytes(assetPath('/models/world.json?v=11'),this.loadAbort.signal).then(bytes=>{const value=JSON.parse(new TextDecoder().decode(bytes));if(!Array.isArray(value.colliders)||!value.actors)throw new Error('小镇地图数据不完整');tracker.update(parts.length,1);return value;});
  const [gltfs,data]=await Promise.all([boundedMap(parts,this.quality?3:2,async(part,i)=>{
   ...
   for(let attempt=0;attempt<2;attempt++){
    ...
    try{const bytes=await fetchBytes(assetPath('/models/'+part+'.glb?v=20260910-11'),this.loadAbort.signal);tracker.update(i,.65);
  ...
  for(const [name,x,y,z,w,h,travel] of [['v11_nobita_slide',-11.44,5.73,4.17,1.04,2,1.07],['v11_closet_slide',-9.3,4.46,4.28,.62,2.1,-.64]] as const){const object=this.world.getObjectByName(name);if(object){const collider={x,y,z,w,d:.065,h,group:name};this.colliders.push(collider);this.slidingDoors.push({object,collider,baseX:x,travel,target:0});}}
 ...
 setMode(mode:ViewMode){this.state.mode=mode;this.walkTarget=null;if(mode==='first'){this.yaw=this.state.floor?Math.PI:0;this.pitch=0;}else{document.exitPointerLock?.();this.distance=this.state.inside?(this.state.floor?10:15):35;this.target.copy(this.playerPosition).add(new THREE.Vector3(0,1,0));}this.layers();this.emit();}
 ...
 updateDoors(dt:number){for(const d of this.slidingDoors){const x=THREE.MathUtils.damp(d.object.position.x,d.target,10,dt);const next={...d.collider,x:d.baseX+x};if(collides(this.playerPosition.x,-this.playerPosition.z,this.playerPosition.y,[next],.23))continue;d.object.position.x=x;d.collider.x=next.x;}}
 layers(){if(!this.world)return;const indoor=this.state.inside;const doll=this.state.mode==='orbit'&&(indoor||this.state.cutaway);const lower=doll&&this.state.floor===0;this.player.visible=this.state.mode==='orbit';
  ...
  set('v11_nobita_slide',!(homeDoll&&this.state.floor===0));set('v11_closet_slide',!(homeDoll&&this.state.floor===0));
 ...
 nearInteraction(){const p=this.playerPosition;const x=p.x,y=-p.z,z=p.y;let near='';let actor:Actor|undefined;
  ...
  if(this.state.house==='home'&&z>2.8&&Math.abs(x-HOUSE.stairX)<.7&&y>6.15)near='下楼 · 客厅';
  ...
  if(this.state.house==='home'&&z>3&&Math.hypot(x+11.44,y-5.73)<1.1)near='开合 · 房间推拉门';
  if(this.state.house==='home'&&z>3&&Math.hypot(x+9.67,y-4.46)<1.4)near='开合 · 哆啦A梦的壁橱';
 ...
 interact(){if(!this.state.ready)return;const {near,actor}=this.nearInteraction();
  ...
  if(near.startsWith('开合')){this.toggleSlidingDoor(near.includes('壁橱')?'v11_closet_slide':'v11_nobita_slide');return;}
