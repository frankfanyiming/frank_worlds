...
export class TownSky {
 ...
 constructor(scene:THREE.Scene,private mobile=false){
  ...
  const material=new THREE.ShaderMaterial({side:THREE.BackSide,transparent:true,depthWrite:false,depthTest:true,
   ...
   fragmentShader:`precision highp float;
    ...
    float fbm(vec3 p){float s=0.,a=.53;for(int i=0;i<${mobile?3:5};i++){s+=a*cloudNoise(p);p=p*2.07+vec3(3.1,7.3,1.8);a*=.48;}return s;}
    void main(){
      ...
      for(int i=0;i<${mobile?2:5};i++){
        float z=float(i)*${mobile?'.55':'.22'};vec3 p=vec3(q,z);
        float cloudSample=fbm(p);
        float density=smoothstep(.46,.72,cloudSample+cover*.17)*${mobile?'.68':'.34'};
        float lightSample=fbm(p+sunDirection*.32);
        float shade=clamp(.65+(cloudSample-lightSample)*1.3+z*.16,.44,1.);
 ...
 snapshot(){return{atmosphericScattering:true,cloudLayers:this.mobile?2:5,cloudTime:this.cloud.material.uniforms.time.value};}
}
