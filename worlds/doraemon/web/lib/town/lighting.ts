...
export class TownInteriorLight {
 ...
 constructor(scene:THREE.Scene,private mobile=false){
  ...
  const sources:[string,number[],number[],number,number,number][]=[
   ...
   ['shizuka living',[-10.4,2.15,20.6],[-12.2,1.1,18.4],2.5,1.6,1.4],
   ['shizuka upper',[-10.4,4.7,19.1],[-12.0,3.5,17.6],2.5,1.6,1.4],
  ];
 ...
 setZone(house:'home'|'shizuka'|null,floor:number,inside:boolean){
  ...
  const bedroom=inside&&house==='home'&&floor===1;
  this.bedroomSun.visible=bedroom;this.bedroomSun.shadow.needsUpdate=bedroom;
  for(const {light} of this.windows){
   const isBedroom=light.name.includes('bedroom'),isShizuka=light.name.includes('shizuka');
   light.visible=inside&&(house==='home'?!isShizuka&&isBedroom===(floor===1):house==='shizuka'&&isShizuka&&light.name.includes('upper')===(floor===1));
  }
  this.practicals.forEach((light,i)=>{light.visible=inside&&(house==='home'?(floor===1?i===0:i===1||i===2):house==='shizuka'&&i===(floor===1?4:3));});
 }
 ...
 snapshot(){return{windowSources:this.windows.length,practicals:this.practicals.length,activeWindows:this.windows.filter(w=>w.light.visible).length,activePracticals:this.practicals.filter(l=>l.visible).length,bedroomShadowActive:this.bedroomSun.visible,daylightIntensity:+this.windows[0].light.intensity.toFixed(3)};}
}
